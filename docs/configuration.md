...
The rest are still owed. #71 collects the default lifetime into the same class,
and it is also where refusal on save arrives: today an invalid value is refused by
the routine that reads it, which is a later moment than the operator typing it.
Each of those settings gets a row here when it lands, and the test is what refuses
one that does not.
