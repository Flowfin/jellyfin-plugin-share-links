...
| `fuzz.yml`                  | Adopted, landed      | `.github/workflows/fuzz.yml`, by #19. Weekly SharpFuzz under libFuzzer over `ShareResolution.Resolve`, which is what `ShareLinksGuestController` hands the path segment to. It runs on a schedule and on dispatch only, so it is adopted and is not a candidate for a required context.                                                               |
...
| `opengrep.yml`              | Adopted, landed      | `.github/workflows/invariants.yml`. #16 held what was left of the seeding and has closed since this row was written.                                                                                                                                                                                                                                  |
...
| `prettier.yml`              | Adopted, landed      | `.github/workflows/prettier.yml`, by #17. `prettier.config.js` is where the two departures from the default are argued.                                                                                                                                                                                                                               |
