...
| `publish-failure-alert.yml` | Adopted, landed      | `.github/workflows/publish-failure-alert.yml`, by #91. It has never fired: no tag has been pushed here, so what it does is designed rather than observed, and the clause of #91 asking that it be seen to fire is still owed.                                               |
