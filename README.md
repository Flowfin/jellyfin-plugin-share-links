...
does: `ReadmeTests.EveryVersionTheReadmeNamesIsTheOneTheManifestCarries`, with
`ReadmeTests.TheReadmeNamesTheVersionRatherThanLeavingItOut` for the other
direction, so the rule cannot be satisfied by naming no version at all. A readme
naming a version the package does not carry sends somebody to install something
that will not load, and the number is in two files rather than one only because a
reader will not open the manifest.
